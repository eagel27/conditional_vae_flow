...
class MADE(nn.Module):
    def __init__(self, latent_dim, cond_dim, hidden_units=[512, 512]):
        ...
        self.fc3 = nn.Linear(hidden_units[1], 2 * self.latent_dim)  # For both shift and log scale
...
class RegressiveFlow(nn.Module):
    ...
    def make_cond_encoder(self):
        return nn.Sequential(
            nn.Flatten(),
            nn.Linear(16 * 16 * 64, 10)  # Example encoder
        )
    ...
    def log_prob(self, x, cond):
        ...
        log_prob = -0.5 * (log_2pi + log_scale + (x - shift) ** 2 / torch.exp(log_scale))
